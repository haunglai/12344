...
def create_answer(qid, user, content):  # 创建答案
    ...
    sql1 = "INSERT INTO t_answer (qid, uid, content) VALUES (%d, (SELECT uid FROM t_user WHERE username='%s'), '%s');" % (
        qid, user, content)
...
def delete_answer_by_id(aid, qid, user):  # 删除答案
    conn = yield async_connect()
    cur = conn.cursor()
    sql = "DELETE FROM t_answer WHERE aid = %d AND qid = %d AND uid=(SELECT uid FROM t_user WHERE username='%s');" % (
        aid, qid, user)
    try:
...
def add_point(aid, qid, user):  # 增加得分
    conn = yield async_connect()
    cur = conn.cursor()
    sql1 = "UPDATE t_user SET point = point + 1 WHERE uid = (SELECT uid FROM t_answer WHERE aid=%d AND qid=%d);" % (
        aid, qid)
    sql2 = "UPDATE t_question SET status=TRUE, adopted_count = adopted_count + 1 WHERE qid = %d AND uid = (SELECT uid FROM t_user WHERE username='%s');" % (
        qid, user)
    try:
...
def get_adopted_count(qid, user):  # 获取得分数据
    conn = yield async_connect()
    cur = conn.cursor()
    sql = "SELECT adopted_count FROM t_question WHERE qid=%d AND uid=(SELECT uid FROM t_user WHERE username='%s')" % (
        qid, user)
    try:
        yield cur.execute(sql)
        data = cur.fetchone()
